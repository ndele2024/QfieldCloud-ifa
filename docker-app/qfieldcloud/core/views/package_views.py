...
class LatestPackageView(views.APIView):
    ...
    def get(self, request, project_id):
        ...
        # Build the set of .qml source names that have been packaged as *_qfield.qml.
        # e.g. "ipe_test_cloud_qfield.qml" → exclude "ipe_test_cloud.qml" from PROJECT_FILEs
        # so QField only receives the packaged version, not the raw source.
        packaged_qml_source_names = set()
        for pf in files_qs.filter(name__endswith=".qml"):
            stem = pf.name[:-4]  # strip .qml
            if stem.endswith("_qfield"):
                packaged_qml_source_names.add(stem[:-7] + ".qml")

        # Include all project files (attachments, JSON, etc.) except:
        # - .qgs/.qgz  → already covered by PACKAGE_FILEs as _qfield.qgz
        # - source .qml plugin files → already packaged as _qfield.qml PACKAGE_FILEs
        project_files_qs = File.objects.filter(
            project_id=project_id,
            file_type=File.FileType.PROJECT_FILE,
        ).exclude(name__endswith=".qgs").exclude(name__endswith=".qgz")
        if packaged_qml_source_names:
            project_files_qs = project_files_qs.exclude(
                name__in=packaged_qml_source_names
            )
        files_qs |= project_files_qs
...
class LatestPackageDownloadFilesView(views.APIView):
    ...
    def get(self, request, project_id, filename):
        ...
        # .qgs/.qgz are served from the package (packaged as _qfield.qgz).
        # All other project files (attachments, plugins, QML, JSON, etc.) are served
        # directly from the original project files.
        is_project_source = not (filename.endswith(".qgs") or filename.endswith(".qgz"))
        if is_project_source and File.objects.filter(
            project_id=project_id,
            file_type=File.FileType.PROJECT_FILE,
            name=filename,
        ).exists():
